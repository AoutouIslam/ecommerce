...
<div id="dropdownDivider" class="hidden absolute z-10 w-44 text-base list-none bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
    <ul class="py-1" aria-labelledby="dropdownDividerButton">
      <li>
        <a href="{{URL::current()."?sort=partinence&category=". $category }}" class="block py-2 px-4 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">partinence</a>
      </li>
      <li>
        <a href="{{URL::current(). "?sort=new&category=". $category }}" class="block py-2 px-4 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">Nouveau</a>
      </li>
      <li>
        <a href="{{URL::current() ."?sort=price_asc&category=". $category }}" class="block py-2 px-4 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">Prix: bas au haut</a>
      </li>
    </ul>
    <div class="py-1">
      <a href="{{URL::current(). "?sort=price_desc&category=". $category}}" class="block py-2 px-4 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">Prix: haut au bas</a>
    </div>
    </div>
      </div>
...
<div class=" grid grid-flow-row grid-cols-2 sm:grid-cols-2 md:grid-cols-2  lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-10 rounded-lg ">
    ...
    <div class="bg-white shadow-lg rounded-lg relative mt-4">
        ...
        @if($data[0] != "")
        <a href="#">
        <img  src="{{ URL::asset($article->image_location . '/images/'. $data[0] ) }}" class="h-40 md:h-80 w-full rounded-lg rounded-tl-lg rounded-tr-lg"/>    </a>

        @else
        <a href="#">
        <img src="{{ URL::asset( 'files/images/error.png' ) }}" class="h-40 md:h-80 w-full rounded-lg rounded-tl-lg rounded-tr-lg"/>    </a>
        @endif
            <!-- div h3 begin--->
        <div class="p-2 md:p-5">
            ...
            <div class="flex flex-row my-1 sm:justify-center grid grid-cols-1 sm:grid-cols-2 text-center">
                ...
                <div class="border-2 border-gray-300 text-gray-800 rounded-full  text-xs text-center  mr-1 my-1 md:my-2">{{$d[0] ."X" . $d[1] . "X" . $d[2] ."CM"}}</div>
            ...
            <div class="flex flex-row my-1 justify-center">
                @foreach ($colors as $c)
                <div class="h-4 w-4 md:h-8 md:w-8 rounded-full shadow-md border border-gray-500 mr-2" style="background-color: {{ $c }};"></div>
                @endforeach
            </div><!-- end div colors-->
            ...
            <div class="flex flex-col  xl:flex-row justify-between product_favori">
               <a href="#" class="justify-center my-1  bg-gradient-to-r from-red-600 to-pink-500 rounded-full py-1 md:py-2 px-4 text-gray-50 text-xs flex flex-row hover:from-pink-600 hover:to-pink-600 favori">
               ...
               <a href="#" class="justify-center my-1 bg-purple-600 rounded-full py-1 md:py-2 px-4 text-gray-50 flex flex-row text-xs hover:bg-purple-700 ">
